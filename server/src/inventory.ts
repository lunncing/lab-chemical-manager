...
import { chemicalCreateSchema, discardSchema, moveSchema } from './validation.js';
...
export function mapChemical(row: Record<string, unknown>) {
  return {
    ...
    inboundAt: String(row.inbound_at), cabinet: row.cabinet as 'A' | 'B', shelf: Number(row.shelf), status: row.status as 'active' | 'discarded',
...
export function inventoryRouter(db: Db, io: SocketServer): Router {
  ...
  router.get('/', asyncRoute((request, res) => {
    ...
    if (req.query.cabinet !== undefined) {
      if (req.query.cabinet !== 'A' && req.query.cabinet !== 'B') throw new HttpError(400, '柜号仅允许 A 或 B', 'VALIDATION_ERROR');
      conditions.push('c.cabinet=?'); params.push(req.query.cabinet);
    }
    if (req.query.shelf !== undefined) {
      const shelf = Number(req.query.shelf); if (!Number.isInteger(shelf) || shelf < 1 || shelf > 5) throw new HttpError(400, '柜层仅允许 1–5', 'VALIDATION_ERROR');
      conditions.push('c.shelf=?'); params.push(shelf);
    }
    if (typeof req.query.search === 'string' && req.query.search.trim()) {
      conditions.push('(c.name LIKE ? OR c.specification LIKE ? OR owner.display_name LIKE ? OR owner.username LIKE ?)');
      const term = `%${req.query.search.trim()}%`; params.push(term, term, term, term);
    }
  ...
  router.post('/', asyncRoute((request, res) => {
    ...
    const committed = transaction(db, () => {
      ...
      const audit = insertAudit(db, { actorId: req.user.id, action: 'inventory_inbound', objectType: 'chemical', objectId: id, summary: `入库 ${input.name} 至 ${input.cabinet}-${input.shelf}`, details: { cabinet: input.cabinet, shelf: input.shelf, ownerId } }, now);
  ...
  router.patch('/:id/move', asyncRoute((request, res) => {
    ...
    const committed = transaction(db, () => {
      ...
      const chemical = getChemical(db, id);
      const audit = insertAudit(db, { actorId: req.user.id, action: 'inventory_move', objectType: 'chemical', objectId: id, summary: `调动 ${current.name}：${current.cabinet}-${current.shelf} → ${input.cabinet}-${input.shelf}`, details: { from: { cabinet: current.cabinet, shelf: current.shelf }, to: { cabinet: input.cabinet, shelf: input.shelf } } }, now);

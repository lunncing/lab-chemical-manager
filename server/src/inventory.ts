...
import { chemicalCreateSchema, chemicalQuerySchema, discardSchema, moveSchema } from './validation.js';
...
export function inventoryRouter(db: Db, io: SocketServer): Router {
  ...
  router.post('/', asyncRoute((request, res) => {
    ...
    const committed = transaction(db, () => {
      ...
      const audit = insertAudit(db, { actorId: req.user.id, action: 'inventory_inbound', objectType: 'chemical', objectId: id, summary: `入库 ${input.name} 至 ${formatLocation(input.cabinet, input.shelf)}`, details: { cabinet: input.cabinet, shelf: input.shelf, ownerId } }, now);

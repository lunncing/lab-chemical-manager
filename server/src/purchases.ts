...
function initialPurchaseStatus(requestType: 'normal' | 'urgent', hazardous: boolean): PurchaseStatus {
  if (requestType === 'urgent') return 'pending_super';
  return hazardous ? 'pending_hazardous' : 'pending_normal';
}

function approvalStage(purchase: Pick<ReturnType<typeof mapPurchase>, 'status' | 'requestType' | 'hazardous'>): ApprovalStage | null {
...
function reviewNotification(status: PurchaseStatus): { category: NotificationCategory; clause: string; label: string } {
  if (status === 'pending_super') return { category: 'purchase_urgent', clause: `u.role='super_admin'`, label: '加急申请' };
  if (status === 'pending_hazardous') return { category: 'hazardous', clause: `u.role IN ('hazardous_buyer','super_admin')`, label: '危险品复核申请' };
  return { category: 'purchase_normal', clause: `u.role IN ('normal_admin','super_admin')`, label: '普通申请' };
}

function requestRecipients(db: Db, status: PurchaseStatus): { userIds: number[]; category: NotificationCategory; label: string } {
  const target = reviewNotification(status);
  return { userIds: eligibleUserIds(db, target.category, target.clause), category: target.category, label: target.label };
}

function approvalTaskWhere(role: AuthedRequest['user']['role']): string | null {
  if (role === 'normal_admin') return `p.status IN ('pending_normal','deferred') AND p.request_type='normal' AND p.hazardous=0`;
  if (role === 'hazardous_buyer') return `(p.status IN ('pending_hazardous','deferred_hazardous') OR (p.status='deferred' AND p.request_type='normal' AND p.hazardous=1))`;
  if (role === 'super_admin') return `p.status IN ('pending_normal','pending_super','pending_hazardous','deferred','deferred_hazardous')`;
  return null;
}

function canReviewStage(role: AuthedRequest['user']['role'], stage: ApprovalStage): boolean {
  return role === 'super_admin' || (stage === 'normal' && role === 'normal_admin') || (stage === 'hazardous' && role === 'hazardous_buyer');
}

function statusAfterEdit(current: ReturnType<typeof mapPurchase>, requestType: 'normal' | 'urgent', hazardous: boolean): PurchaseStatus {
  const hazardousReviewStage = current.status === 'pending_hazardous' || current.status === 'deferred_hazardous'
    || (current.status === 'deferred' && current.requestType === 'normal' && current.hazardous);
  if (hazardousReviewStage) {
    if (current.requestType === 'normal' && requestType === 'urgent') return 'pending_super';
    return 'pending_hazardous';
  }
  return initialPurchaseStatus(requestType, hazardous);
}
...
export function purchasesRouter(db: Db, io: SocketServer): Router {
  ...
  router.get('/', asyncRoute((request, res) => {
    ...
    const rows = db.prepare(`${selectPurchase} WHERE ${clauses.join(' AND ')} ORDER BY p.id DESC LIMIT 500`).all(...params) as Array<Record<string, unknown>>;
    res.json({ purchases: rows.map(mapPurchase) });
  }));
  router.post('/', asyncRoute((request, res) => {
    ...
    const status = initialPurchaseStatus(input.requestType, input.hazardous);
    const recipients = requestRecipients(db, status);
    const committed = transaction(db, () => {
      ...
      const notifications = insertNotifications(db, { userIds: recipients.userIds, category: recipients.category, title: `新的${recipients.label}`, body: `${req.user.displayName} 申请采购 ${input.chemicalName}`, objectType: 'purchase', objectId: id }, now);
  ...
  router.patch('/:id', asyncRoute((request, res) => {
    ...
    if (!['pending_normal', 'pending_super', 'pending_hazardous', 'deferred', 'deferred_hazardous'].includes(String(current.status))) throw new HttpError(409, '当前状态不能修改', 'CONFLICT');
    const requestType = input.requestType ?? current.requestType as 'normal' | 'urgent';
    const hazardous = input.hazardous ?? current.hazardous;
    const status = statusAfterEdit(current, requestType, hazardous);
    const recipients = requestRecipients(db, status);
    ...
    const committed = transaction(db, () => {
      const result = db.prepare(`UPDATE purchases SET chemical_name=?,specification=?,purpose=?,hazardous=?,request_type=?,status=?,approval_comment=NULL,decided_at=NULL,version=version+1,updated_at=? WHERE id=? AND version=?`).run(
        ...
        Number(hazardous), requestType, status, now, id, input.version);
      ...
      const notifications = insertNotifications(db, { userIds: recipients.userIds, category: recipients.category, title: `${recipients.label}已修改`, body: `${req.user.displayName} 修改了 ${purchase.chemicalName} 的申请`, objectType: 'purchase', objectId: id }, now);
      return { purchase, audit, notifications };
    });
  ...
  router.post('/:id/withdraw', asyncRoute((request, res) => {
    ...
    if (!['pending_normal', 'pending_super', 'pending_hazardous', 'deferred', 'deferred_hazardous'].includes(String(current.status))) throw new HttpError(409, '当前状态不能撤销', 'CONFLICT');
    const stage = approvalStage(current);
    if (!stage) throw new HttpError(409, '当前状态不能撤销', 'CONFLICT');
    const pendingStatus: PurchaseStatus = stage === 'super' ? 'pending_super' : stage === 'hazardous' ? 'pending_hazardous' : 'pending_normal';
    const recipients = requestRecipients(db, pendingStatus);
    const now = new Date().toISOString();
    const committed = transaction(db, () => {
      ...
      const notifications = insertNotifications(db, { userIds: recipients.userIds, category: recipients.category, title: '采购申请已撤销', body: `${req.user.displayName} 撤销了 ${purchase.chemicalName} 的申请`, objectType: 'purchase', objectId: id }, now);
      return { purchase, audit, notifications };
    });
    emitCommitted(io, 'purchase:changed', committed.purchase, committed.audit, committed.notifications); res.json({ purchase: committed.purchase });
  }));
  ...
  router.post('/:id/decision', asyncRoute((request, res) => {
    ...
    const stage = approvalStage(current);
    if (!stage) throw new HttpError(409, '当前状态不能审批', 'CONFLICT');
    if (!canReviewStage(req.user.role, stage)) {
      const message = stage === 'super' ? '加急申请第一阶段仅超级管理员可审批'
        : stage === 'hazardous' ? '危险品复核仅审批与危险采购人或超级管理员可处理'
          : '普通非危险申请仅审批与普通采购人或超级管理员可处理';
      throw new HttpError(403, message, 'FORBIDDEN');
    }
    const now = new Date().toISOString();
    const entersHazardousReview = input.decision === 'approved' && stage === 'super' && current.hazardous;
    const nextStatus: PurchaseStatus = entersHazardousReview ? 'pending_hazardous'
      : input.decision === 'deferred' ? (stage === 'hazardous' ? 'deferred_hazardous' : 'deferred')
        : input.decision;
    const committed = transaction(db, () => {
      const result = db.prepare('UPDATE purchases SET status=?,approval_comment=?,decided_at=?,version=version+1,updated_at=? WHERE id=? AND status=? AND version=?').run(
        nextStatus, input.comment ?? null, now, now, id, current.status, input.version,
      );
      if (!result.changes) throw new HttpError(409, '申请已被其他人修改', 'CONFLICT');
      const purchase = getPurchase(db, id);
      if (nextStatus === 'approved' && purchase.requestType === 'normal' && !purchase.hazardous) {
      ...
      const audit = entersHazardousReview
        ? insertAudit(db, {
          actorId: req.user.id, action: 'purchase_hazardous_review_requested', objectType: 'purchase', objectId: id,
          summary: `加急危险品老师初审通过，进入危险品复核：${purchase.chemicalName}`,
          details: { stage: 'super', fromStatus: current.status, status: nextStatus, comment: input.comment ?? null },
        }, now)
        : insertAudit(db, {
          actorId: req.user.id, action: `purchase_${input.decision}`, objectType: 'purchase', objectId: id,
          summary: `${stage === 'hazardous' ? '危险品复核' : ''}${labels[input.decision]}采购申请：${purchase.chemicalName}`,
          details: { stage, fromStatus: current.status, status: nextStatus, comment: input.comment ?? null },
        }, now);
      const notifications = entersHazardousReview
        ? insertNotifications(db, {
          userIds: eligibleUserIds(db, 'hazardous', `u.role IN ('hazardous_buyer','super_admin')`), category: 'hazardous',
          title: '危险品复核任务', body: `${purchase.chemicalName} 已通过老师加急初审，请进行危险品复核`, objectType: 'purchase', objectId: id,
        }, now)
        : insertNotifications(db, {
          userIds: eligibleUserIds(db, 'approval', 'u.id=?', [current.applicant.id]), category: 'approval',
          title: `采购申请${labels[input.decision]}`, body: `${purchase.chemicalName} 的申请已${labels[input.decision]}`, objectType: 'purchase', objectId: id,
        }, now);

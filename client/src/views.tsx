...
export function PurchasesView({ user, revision, onChanged }: { user: UserView; revision: number; onChanged: () => void }) {
  const [purchases, setPurchases] = useState<Purchase[]>([]); const [mode, setMode] = useState<PurchaseViewMode>('all'); const [status, setStatus] = useState(''); const [kind, setKind] = useState(''); const [hazardous, setHazardous] = useState(''); const [error, setError] = useState(''); const [showCreate, setShowCreate] = useState(false);
...
export function NotificationsView({ revision, onChanged }: { revision: number; onChanged: (unread?: number) => void }) {
  const [items, setItems] = useState<NotificationItem[]>([]); const [prefs, setPrefs] = useState<Array<{ category: string; enabled: boolean }>>([]); const [category, setCategory] = useState<NotificationCategoryFilter>(''); const [readState, setReadState] = useState<NotificationReadFilter>('all'); const [error, setError] = useState('');

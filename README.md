...
```text
lab-chemical-manager-v1.8.0-windows-deploy.zip
```
...
V1.8.0 当前验证结果：52 个测试文件、183/183 自动测试、TypeScript 检查、生产构建、完整 acceptance 和性能基准均通过。性能数字来自当前开发机，只作为回归基线，不代表目标服务器等效结果。V1.8.0 尚无独立 DSH 审查原件，不沿用 V1.7.0 的审查结论。

...
- 直接/代入库可选填并校验 CAS 号；库存可按 CAS 搜索，任意已登录且启用的成员可通过应用内弹窗更正活跃药品的名称、规格、CAS 号和入库时间。
...
- 超级管理员可安全删除账号的登录身份：原用户名、密码、会话和个人消息状态会移除，姓名继续显示在历史药品、采购、代入库、邀请和审计中；不能删除自己或最后一个启用的超级管理员。
...
账号“删除”是不可逆的登录身份删除：系统会把原用户名替换为不可预测的墓碑值，把密码替换为不可知随机密码的 scrypt 哈希，停用账号并删除全部会话、个人通知和通知偏好。原 `display_name` 姓名会保留，用于药品归属人/入库操作人、调动、采购、代入库、邀请码创建人和公开审计 actor 等历史归属；删除审计本身不写入真实姓名。删除后的原用户名可以由新成员重新注册，自删、最后一个启用的超级管理员、重复或并发删除、历史外键等保护保持有效。

V1.9.1 不会猜测或自动补写姓名：如果账号已被旧版本删除，且 `display_name` 已经变成“已删除用户 #ID”，原姓名无法从系统可靠恢复，该占位值会原样保留。
...
V1.9.1 当前验证结果：58 个测试文件、210/210 自动测试、TypeScript 检查、生产构建、完整 acceptance、真实 HTTP 和性能基准均通过；production audit 为 0。`codex/gpt-5.6-sol` 完整读取并审查 16/16 个变更文件，结论为 `PASS`；用户已确认候选“通过”。性能数字来自当前开发机，只作为回归基线，不代表目标服务器等效结果。本次验收仅使用内存或临时数据库和临时端口；远程真实已录入数据库未被访问，部署前仍须按现场流程单独验证。
